...
from etl_pipeline.transform import (
    delete_columns, drop_null_rows, fill_missing_values, rename_columns, 
    filter_rows, add_new_column, change_column_type, standardize_date_column, rename_year_columns
)
...
def test_delete_columns(sample_dataframe: pd.DataFrame) -> None:
    ...
    :return: None
    """
    columns_to_delete = ['Column2', 'Column4']  # Column4 does not exist, should be ignored
...
def test_fill_missing_values(sample_dataframe: pd.DataFrame) -> None:
    """
    Test the fill_missing_values function for filling missing values with a specified value.
...
def test_change_column_type(sample_dataframe: pd.DataFrame) -> None:
    """
    Test the change_column_type function for changing the data type of a specified column.
...
def test_standardize_ym_date_column(sample_ym_date_dataframe: pd.DataFrame) -> None:
    """
    Test the standardize_date_column function for handling YYYYMM date format.

    :param sample_ym_date_dataframe: Sample DataFrame with YYYYMM date columns for testing.
    :type sample_ym_date_dataframe: pd.DataFrame
    :return: None
    """
    # Expected DataFrame after date standardization
    expected_data = {
        'Date': pd.to_datetime(['1958-01-01', '1958-02-01', '1958-03-01', '1958-04-01']),
        'Value': [1.1, 2.2, 3.3, 4.4]
    }
...
def test_standardize_md_date_column(sample_md_date_dataframe: pd.DataFrame) -> None:
    """
    Test the standardize_date_column function for handling DMM/DD/YYYY date format.

    :param sample_md_date_dataframe: Sample DataFrame with DMM/DD/YYYY date columns for testing.
    :type sample_md_date_dataframe: pd.DataFrame
    :return: None
    """
    # Expected DataFrame after date standardization
    expected_data = {
        'Date': pd.to_datetime(['1992-12-17', '1993-01-01', '1993-02-28', '1993-03-15']),
        'Value': [1.1, 2.2, 3.3, 4.4]
    }
    expected_df = pd.DataFrame(expected_data)
