...
def test_num_of_crop_categories(create_dbCursor):
    ...
    create_dbCursor.execute("SELECT DISTINCT Item from yield_data")
...
def test_is_table_crop1_present(create_dbCursor):
    create_dbCursor.execute("SELECT name FROM sqlite_master WHERE type='table';")
    all_table_list = create_dbCursor.fetchall()
    assert ('area_harvested_data',) in all_table_list

def test_is_table_crop2_present(create_dbCursor):
    create_dbCursor.execute("SELECT name FROM sqlite_master WHERE type='table';")
    all_table_list = create_dbCursor.fetchall()
    assert ('production_data',) in all_table_list

def test_is_table_crop3_present(create_dbCursor):
    create_dbCursor.execute("SELECT name FROM sqlite_master WHERE type='table';")
    all_table_list = create_dbCursor.fetchall()
    assert ('yield_data',) in all_table_list
